...
#Plot of our data. Note that each point represents the rating of the game's white and black player
...
#Plots the data, but sections the points into colors based on the grid boundaries given by white_elo_boundaries
...
#Plot of the data after imposing the white ratings into grids
plot_white_boundaries_only(game_info_df, white_elo_boundaries)


#Plot of the data after imposing the white and black ratings into grids and subgrids respectively
plot_elo_boundaries(game_info_df, white_elo_boundaries, black_elo_boundaries_list)




#Tells us whether or not we want to include draws in our model, and whether random forest will be simulated.
#Set use_rf_model2 to False if we want to run a SVM model, and true for random forest
include_draws2 = False; use_rf_model2 = True


#num_trees2 is the number of trees that random forest will use
num_trees2 = list(range(100, 151))


#cost2 is the cost values that SVM will use
cost2 = list((1, 10, 50, 100, 150)) 


#These columns will be the predictor variables
chosen_columns2 = list(('Delta Rating', '40th FEN Eval', 'Sum Eval', 'Total Half Moves'))


########

#These lines below are commented out because their functions take a significant amount of time to run. 
#Uncomment them if running the models is desired.
