    ...
    for i in range(len(white_ratings)):
        ...
        for j in range(len(white_elo_boundaries) - 1):
            ...
            if white_min <= white_ratings[i] <= white_max:
                ...
                #Assign black boundaries for this white boundary
                black_bounds = black_elo_boundaries[j]

                #Assign a unique colour for each sub-boundary 
...
def get_best_svm_model(pd_df, cost_values, chosen_columns, include_draws):
    ...
    if include_draws:
        # Set the categories explicitly to handle cases where some categories might be missing
        results = pd_df['Result'].astype('category').cat.set_categories([-1, 0, 1])
    else:
        results = pd_df['Result'].astype('category').cat.set_categories([-1, 1])

    
    #Extract categorical columns
    categorical_cols = pd_df[chosen_columns].select_dtypes(include=['object', 'category']).columns.tolist()


    #Extract numerical columns
    numerical_cols = pd_df[chosen_columns].select_dtypes(include=['int64', 'float64']).columns.tolist()
    ...
    for c in cost_values:
        ...
        #The Pipeline function first merges the data together and then feeds it into SVM 
        svm_pipeline = Pipeline(steps=[
            ('preprocessor', preprocessor),
            ('classifier', SVC(C=c, random_state=1000, probability=True))
        ])

        #Perform cross-validation and calculate the mean error using negative log loss
        error = -np.mean(cross_val_score(svm_pipeline, pd_df[chosen_columns], results, cv=3, scoring='neg_log_loss'))
        

        model_list.append(svm_pipeline)

        #Store the cost and error values
        errors.append(error)
        cost_used.append(c)  
    ...
    best_model.fit(pd_df[chosen_columns], results)
...
def get_best_rf_model(pd_df, trees, chosen_columns, include_draws):
    ...
    #Initialize lists to store models and errors
    model_list = []
    errors = []
    ...
    for tree_count in trees:
        ...
        #The Pipeline function first merges the data together and then feeds it into random forest 
        rf_pipeline = Pipeline(steps=[
            ('preprocessor', preprocessor),
            ('classifier', RandomForestClassifier(n_estimators=tree_count, random_state=1000, class_weight='balanced'))
        ])

        #Perform cross-validation and calculate error
        error = -np.mean(cross_val_score(rf_pipeline, pd_df[chosen_columns], results, cv=3, scoring='neg_log_loss'))
        
        #Append model and errors to lists
        model_list.append(rf_pipeline)
        errors.append(error)
    ...
    #Retrieve the number of trees used in the best random forest model
    best_tree = trees[best_model_index]
...
def sample_results(model_results_df, include_draws, is_rf_model, num_rows_to_sample):
    ...
    random_indices = random.sample(range(model_results_df.shape[0]), min(num_rows_to_sample, model_results_df.shape[0]))

    model_df_subset = model_results_df.iloc[random_indices]
...
#np.savetxt("rf_model_summary_array2.csv", rf_model_summary_array, delimiter=",", fmt='%.4f')
#np.savetxt("svm_model_summary_array2.csv", svm_model_summary_array, delimiter=",", fmt='%.4f')





svm_results = pd.read_csv("C:\\Users\\ryne0\\Downloads\\svm_model_summary_array2.csv", dtype=float, header = None)
rf_results = pd.read_csv("C:\\Users\\ryne0\\Downloads\\rf_model_summary_array2.csv", dtype=float, header = None)



plot_model_errors_with_draws(rf_results, True, False, 10)
sample_results(rf_results, False, False, 10)
...
#Blocking by time control. Chose 300+0 to demonstrate the improved performance of the models.
#Time control is a significant lurking variable in the performance of the models as it significantly affects the
#performance of the predictor variables
...
#Have to control for the lurking variables: time control and whether the game was in a tournament or not (online or not).
#Could also control on how the player lost. Would likely lead to more accurate predictions.
#Can show that tournament games will have less error because the players care more about the game's outcome



#new_int_array's columns: result, white elo, black elo, delta_avg_elo, the fortieth_fen_eval, the sum_eval_value for the game, the total
#number of moves played by both sides in the game, which section each white elo belongs to, which subsection each black
#elo belongs to within each white elo section, an indicator of whether that row of data will be used for training or testing
#need to also make a delta_elo column

#Note that white grid and black grid values start at 0




#Save players' names to query them later, and save the date to watch rating progression over time,
#Could also predict based on different time controls

#could maybe make a heatmap with the boundaries of the grids plotted, and the errors of the models





#Pivot Tables: Create pivot tables to summarize game outcomes based on different criteria (e.g., player color, opening)

#Could see if 1.d4 has increased in popularity after the releasing of queens gambit, and could examine diff in its popularity based on rating groups
